...
import { handleCreateQuestion } from "../../features/questionsSlice";
import { toast } from "react-toastify";
import { handleGetListModulesAvailable, handleGetModuleUnits } from "../../features/modulesSlice";
import { handleGetAllTopics, handleTopicDetails } from "../../features/topicsSlice";
...
// fresh blank options for reset
const freshOptions = () => ([
  { text: "", explanation: "", videoUrl: "", videoFile: null, isCorrect: false },
  { text: "", explanation: "", videoUrl: "", videoFile: null, isCorrect: false },
]);

const CreateQuestion = ({ topicId , unitId , id }) => {
  const [activeTab, setActiveTab] = useState("editor"); // "editor" | "preview" | "assets"
  ...
  const [keywords, setKeywords] = useState(""); // CSV
  const [helpText, setHelpText] = useState("");
  const [points, setPoints] = useState("1");

  // status select + tags multiselect (array)
  const [status, setStatus] = useState("active");
  const [tags, setTags] = useState([]); // array of strings

  const { create_question_loading } =
    useSelector((state) => state?.questions) || { create_question_loading: false };

  const dispatch = useDispatch();
  const {list_module_available , module_units} = useSelector(state => state?.modules)
  const {topics_list} = useSelector(state => state?.topics);  
   ...
   useEffect(() => {
    ...
    useEffect(() => {
      dispatch(handleGetAllTopics({unit_id : unitId}))
    } ,[unitId])


   useEffect(() => {
    console.log(list_module_available , module_units , topics_list)
   } , [list_module_available, module_units , topics_list])

  const addOption = () =>
    setOptions([...options, { text: "", explanation: "", videoUrl: "", videoFile: null, isCorrect: false }]);
  ...
  const handleVideoFileUpload = (index, file) => {
    if (file && file.type.startsWith("video/")) {
      ...
      const newOptions = [...options];
      newOptions[index].videoFile = file;
      newOptions[index].videoUrl = "";
      setOptions(newOptions);
    }
  };

  const colors = { primary: "#0F7490", text: "#202938" };
  ...
  const breadcrumbs = [
    ...
    { label: list_module_available?.data?.modules?.find(item => item?.module_id == id)?.subject_name, href: "#" },
    { label: module_units?.data?.units?.find(item => item?.unit_id== unitId)?.unit_name, href: "#" },
    { label: topics_list?.data?.topics?.find(item => item?.topic_id == topicId)?.topic_name, href: "#" },
  ...
  const buildPayload = () => {
    ...
    if (base.question_type === "essay") {
      return { ...base, model_answer: (modalAnswer || "").trim() };
    }
    return { ...base, options: mapOptionsForApi() };
  };
  ...
  async function handleSubmit() {
    ...
    try {
      const res = await dispatch(handleCreateQuestion(payload)).unwrap();
      if (res?.status === "success") {
        toast.success(res?.message || "Question created successfully");
        resetForm(); // <<< CLEAR EVERYTHING AFTER SUCCESS
      } else {
        toast.error(res || "Failed to create question");
      }
    } catch (e) {
  ...
  return (
    <div className="min-h-screen bg-[#F9FAFC] p-6">
      <div className="max-w-7xl mx-auto">
            ...
            <div className="flex items-center space-x-2">
              <div
                className={`px-3 py-1 rounded-full text-sm font-medium ${
                  questionType === "mcq" ? "bg-[#8B5CF6]/10 text-[#8B5CF6]" : "bg-[#0F7490]/10 text-[#0F7490]"
                }`}
              >
                {questionType === "mcq" ? "Multiple Choice" : questionType === "trueFalse" ? "True/False" : "Essay Question"}
              </div>
            </div>
          }
        />
          ...
          <div className="flex items-center space-x-1 p-2">
            <button
              ...
              className={`flex items-center space-x-2 px-4 py-2 rounded-lg font-medium transition-all duration-200 ${
                activeTab === "editor" ? "bg-[#0F7490] text-white shadow-md" : "text-[#202938]/60 hover:text-[#202938] hover:bg-gray-50"
              }`}
            >
            ...
            <button
              ...
              className={`flex items-center space-x-2 px-4 py-2 rounded-lg font-medium transition-all duration-200 ${
                activeTab === "preview" ? "bg-[#0F7490] text-white shadow-md" : "text-[#202938]/60 hover:text-[#202938] hover:bg-gray-50"
              }`}
            >
            ...
            {/* <button
              ...
              className={`flex items-center space-x-2 px-4 py-2 rounded-lg font-medium transition-all duration-200 ${
                activeTab === "assets" ? "bg-[#0F7490] text-white shadow-md" : "text-[#202938]/60 hover:text-[#202938] hover:bg-gray-50"
              }`}
            >
              ...
              <>
                  ...
                  <div className="grid grid-cols-3 gap-4">
                    <button
                      ...
                      className={`p-4 rounded-lg border-2 transition-all duration-200 text-left ${
                        questionType === "mcq" ? "border-[#0F7490] bg-[#0F7490]/5" : "border-gray-200 hover:border-gray-300"
                      }`}
                    >
                      <h3 className="font-medium text-[#202938]">Multiple Choice</h3>
                      <p className="text-sm text-[#202938]/60">Create questions with multiple answer options</p>
                    </button>

                    <button
                      onClick={() => handleQuestionTypeChange("trueFalse")}
                      className={`p-4 rounded-lg border-2 transition-all duration-200 text-left ${
                        questionType === "trueFalse" ? "border-[#0F7490] bg-[#0F7490]/5" : "border-gray-200 hover:border-gray-300"
                      }`}
                    >
                      <h3 className="font-medium text-[#202938]">True/False</h3>
                      <p className="text-sm text-[#202938]/60">Create simple true or false questions</p>
                    </button>

                    <button
                      onClick={() => handleQuestionTypeChange("essay")}
                      className={`p-4 rounded-lg border-2 transition-all duration-200 text-left ${
                        questionType === "essay" ? "border-[#0F7490] bg-[#0F7490]/5" : "border-gray-200 hover:border-gray-300"
                      }`}
                    >
                      <h3 className="font-medium text-[#202938]">Essay</h3>
                      <p className="text-sm text-[#202938]/60">Create simple essay questions</p>
                    </button>
                  </div>
                </Card>
                ...
                {(questionType === "mcq" || questionType === "trueFalse") && (
                  <Card
                    ...
                    extra={
                      questionType === "mcq" && (
                        <Button type="text" onClick={addOption} className="text-[#0F7490] hover:bg-[#0F7490]/10">
                  ...
                  >
                    <div className="space-y-6">
                      <p className="text-sm text-[#202938]/60 mb-4">
                        Enter answer options and mark the correct one. Add explanations to help students understand.
                      </p>
                      ...
                      {options.map((option, index) => (
                        <div
                          ...
                          className={`p-4 rounded-lg border-2 transition-all duration-200 ${
                            option.isCorrect ? "border-[#0F7490] bg-[#0F7490]/5" : "border-gray-200 hover:border-gray-300"
                          }`}
                        >
                          <div className="flex items-start space-x-3 mb-4">
                            ...
                            <div className="relative mt-3">
                              ...
                              <div
                                className={`w-5 h-5 rounded-full border-2 transition-all cursor-pointer ${
                                  option.isCorrect ? "border-[#0F7490] bg-[#0F7490]" : "border-gray-300"
                                }`}
                            ...
                            <div className="flex-1">
                              <div className="flex items-center space-x-2 mb-2">
                                <span className="text-sm font-medium text-[#202938]">Option {index + 1}</span>
                                {option.isCorrect && (
                                  <span className="bg-[#0F7490] text-white text-xs px-2 py-1 rounded-full">Correct Answer</span>
                                )}
                              </div>
                              <Input
                            ...
                            {/* Remove */}
                            {options.length > 2 && (
                              <Button type="text" onClick={() => removeOption(index)} className="text-red-500 hover:bg-red-50 mt-8">
                            ...
                            <div>
                              ...
                              <div className="border border-gray-200 rounded-lg overflow-hidden">
                                ...
                                <div className="p-4 space-y-4">
                                    ...
                                    <Input
                                      ...
                                      onChange={(e) => {
                                        ...
                                        if (e.target.value && option.videoFile) {
                                          ...
                                          newOpts[index].videoFile = null;
                                          setOptions(newOpts);
                                        }
                                      }}
                                      error={option.videoUrl && !isValidVideoUrl(option.videoUrl) ? "Please enter a valid URL" : null}
                                  ...
                                  {/* File */}
                                  <div>
                                    <label className="block text-xs font-medium text-gray-600 mb-2">Upload Video File</label>
                                      ...
                                      <div className="relative">
                                        ...
                                        <div
                                          className={`border-2 border-dashed rounded-lg p-4 text-center transition-colors ${
                                            option.videoUrl ? "border-gray-200 bg-gray-50 cursor-not-allowed" : "border-gray-300 hover:border-blue-400 hover:bg-blue-50 cursor-pointer"
                                          }`}
                                        >
                                          <Upload className={`w-8 h-8 mx-auto mb-2 ${option.videoUrl ? "text-gray-400" : "text-gray-500"}`} />
                                          <p className={`text-sm ${option.videoUrl ? "text-gray-400" : "text-gray-600"}`}>
                                            {option.videoUrl ? "Clear URL to upload file" : "Click to upload video file"}
                                          </p>
                                          <p className={`text-xs mt-1 ${option.videoUrl ? "text-gray-400" : "text-gray-500"}`}>MP4, WebM, AVI, MOV (Max 100MB)</p>
                                        </div>
                                      </div>
                                    ) : (
                                      <div className="border border-green-200 bg-green-50 rounded-lg p-3">
                                        <div className="flex items-center justify-between">
                                          <div className="flex items-center space-x-2">
                                            ...
                                            <span className="text-sm text-green-800 font-medium">{option.videoFile.name}</span>
                                            <span className="text-xs text-green-600">({(option.videoFile.size / (1024 * 1024)).toFixed(2)} MB)</span>
                                          </div>
                                          <button
                                            onClick={() => {
                                              const newOpts = [...options];
                                  ...
                                  {/* Success */}
                                  {((option.videoUrl && isValidVideoUrl(option.videoUrl)) || option.videoFile) && (
                ...
                <div className="space-y-6">
                  {question ? (
                      ...
                      <div className="space-y-4">
                            ...
                            <div className="mt-3">
                              <span
                                className={`inline-flex items-center px-3 py-1 rounded-full text-sm font-medium ${
                                  difficulty === "easy" ? "bg-green-100 text-green-800" : difficulty === "medium" ? "bg-yellow-100 text-yellow-800" : "bg-red-100 text-red-800"
                                }`}
                              >
                        ...
                        {questionType === "mcq" && options.length > 0 && (
                          <div className="space-y-3">
                            ...
                            {options.map((option, index) => (
                              <div key={index} className={`p-3 border rounded-lg ${option.isCorrect ? "border-green-500 bg-green-50" : "border-gray-200"}`}>
                                <div className="flex items-center space-x-2">
                                  <span className="font-medium">{String.fromCharCode(65 + index)}.</span>
                                  <span>{option.text || `Option ${index + 1}`}</span>
                                  {option.isCorrect && <span className="text-green-600 text-xs font-medium">(Correct Answer)</span>}
                                </div>
                    ...
                    <div className="text-center py-12">
                      ...
                      <h3 className="text-lg font-medium text-gray-600 mb-2">No Question to Preview</h3>
                      <p className="text-gray-500">Start writing your question to see the preview</p>
                    </div>
                  )}
                </div>
              </Card>
                    ...
                    <div className="space-y-4">
                      <div>
                        ...
                        {options.some((opt) => opt.videoFile || opt.videoUrl) ? (
                              ...
                              return (
                                <div key={index} className="border border-gray-200 rounded-lg p-4">
                                  <div className="flex items-center justify-between">
                                    ...
                                    <div className="flex items-center space-x-2">
                                      {option.videoFile && <span className="bg-green-100 text-green-800 text-xs px-2 py-1 rounded-full">Uploaded</span>}
                                      {option.videoUrl && <span className="bg-blue-100 text-blue-800 text-xs px-2 py-1 rounded-full">URL</span>}
                                    </div>
                                  </div>
                          ...
                          <div className="text-center py-8 border-2 border-dashed border-gray-200 rounded-lg">
                            ...
                            <p className="text-gray-500">No video assets uploaded</p>
                            <p className="text-sm text-gray-400 mt-1">Upload videos in the Editor tab under answer options</p>
                          </div>
                        )}
                      </div>
                      ...
                      <div className="bg-gray-50 p-4 rounded-lg">
                        ...
                        <div className="grid grid-cols-2 gap-4">
                          ...
                          <div className="text-center">
                            <div className="text-2xl font-bold text-[#8B5CF6]">{options.filter((opt) => opt.videoUrl && !opt.videoFile).length}</div>
                              ...
                              <div className="text-lg font-semibold text-gray-700">
                                {(
                                  options
                                    ...
                                    .reduce((total, opt) => total + opt.videoFile.size, 0) /
                                  (1024 * 1024)
                                ).toFixed(2)}{" "}
                                MB
                              </div>
          ...
          <div className="space-y-6 sticky top-0">
            <Card title="Question Settings" className="p-6">
              <div className="space-y-6">
                ...
                {/* Hint, Keywords */}
                <TextArea label="Hint" placeholder="Add a short hint to guide students (optional)..." rows={2} value={hint} onChange={(e) => setHint(e.target.value)} />
                <Input
                  label="Keywords"
                    ...
                    {difficultyOptions.map((option) => (
                      <button
                        ...
                        className={`p-3 rounded-lg font-medium transition-all duration-300 transform hover:scale-105 ${
                          difficulty === option.value ? `bg-gradient-to-r ${option.color} text-white shadow-lg` : "bg-gray-100 text-gray-600 hover:bg-gray-200"
                        }`}
                      >
                        {option.label}
                      </button>
            ...
            <div className="space-y-6 sticky top-0">
                    ...
                    <>
                      ...
                      <div className="flex items-center justify-between">
                        ...
                        <span className="font-medium text-[#0F7490]">
                          {options.findIndex((opt) => opt.isCorrect) !== -1 ? `Option ${options.findIndex((opt) => opt.isCorrect) + 1}` : "None Selected"}
                        </span>
                      </div>
                      <div className="flex items-center justify-between">
                      ...
                      <div className="flex items-center justify-between">
                        ...
                        <span className="font-medium text-[#C9AE6C]">{options.filter((opt) => (opt.videoUrl || "").trim() || opt.videoFile).length}</span>
                      </div>
                      <div className="flex items-center justify-between">
                        <span className="text-sm text-[#202938]/60">Video Files</span>
                        <span className="font-medium text-[#8B5CF6]">{options.filter((opt) => opt.videoFile).length}</span>
                      </div>
                    </>
                  ) : (
                    <div className="flex items-center justify-between">
                      <span className="text-sm text-[#202938]/60">Model Answer</span>
                      <span className="font-medium text-[#0F7490]">{(modalAnswer || "").trim().length > 0 ? "Added" : "None"}</span>
                    </div>
                  )}
              ...
              <Card className="p-6">
                <div className="text-center">
                  <div className={`w-16 h-16 mx-auto rounded-full flex items-center justify-center mb-4 ${isFormValid ? "bg-green-100" : "bg-gray-100"}`}>
                    {isFormValid ? <CheckCircle2 className="w-8 h-8 text-green-600" /> : <AlertCircle className="w-8 h-8 text-gray-400" />}
                  </div>
        ...
        <div className="mt-8 flex items-center justify-between bg-white rounded-xl p-6 shadow-sm border border-gray-100">
          <div className="flex items-center space-x-4">
            ...
            <Button type="default" onClick={() => message.info("Saved as draft locally (wire as needed).")}>
              Save as Draft
            </Button>
          </div>
          <Button
            type="primary"
            size="large"
            disabled={!isFormValid || create_question_loading}
            onClick={handleSubmit}
          >
            <SaveIcon className="w-4 h-4 mr-2" />
            {create_question_loading ? "Saving..." : "Save Question"}
          </Button>
        </div>
      </div>
      ...
      {/* Preview modal */}
      {questionType === "mcq" ? (
        <Modal footer={null} open={prevModal} className=" w-full  md:!w-[40%]" onCancel={() => setPrevModal(!prevModal)}>
          <div className="p-6">
            <h3 className="text-lg font-semibold mb-4">Question Preview</h3>
            <div className="space-y-4">
              <div>
                <p className="font-medium text-gray-800">{question || "What is 2 + 2?"}</p>
              </div>
                              ...
                              option.videoUrl && (() => {
                                ...
                                return (
                                  <div className="bg-gray-100 p-3 rounded-lg">
                                    <a href={option.videoUrl} target="_blank" rel="noopener noreferrer" className="text-blue-600 underline text-sm">
