...
function EditRevModal({
  ...
  // Load reviewer into form when it changes
  useEffect(() => {
    if (!reviewer) return;
    form.setFieldsValue({
      name: reviewer.name || "",
      email: reviewer.email || "",
      reviewSubject: reviewer.reviewSubject || "",
      assignedTeachers: reviewer.assignedTeachers || [],
      status: reviewer.status || "Pending",
      sendInvite: false,
    });
    setAvatarPreview(reviewer.avatar || null);
  }, [reviewer, form]);
  ...
  const handleFinish = async (values) => {
    ...
    try {
      ...
      if (typeof onSubmit === "function") {
        ...
        console.log("Edit reviewer payload:", payload);
      }
      message.success("Reviewer updated successfully");
      onCancel && onCancel();
    } catch (e) {
      message.error("Failed to update reviewer. Please try again.");
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleDelete = async () => {
    if (!onDelete || !reviewer) return;
    setLoading(true);
    try {
      await onDelete(reviewer);
      message.success("Reviewer deleted");
      onCancel && onCancel();
    } catch (e) {
        ...
        <div className="bg-background">
          ...
          <div className="mb-6">
            ...
            <p className="text-gray-600">
              Update reviewer profile and assignments.
            </p>
          </div>
                  ...
                  <div className="mt-4 p-4 bg-white rounded-xl border border-gray-200 shadow-sm">
                    <div className="flex items-center gap-3">
                      {avatarPreview ? (
                        ...
                        >
                          {(
                            initials(form.getFieldValue("name")) ||
                            initials(reviewer?.name || "NA")
                          )
                            .slice(0, 2)
                            .toUpperCase()}
                        </div>
                    ...
                    <div className="mt-3 flex gap-2">
                      ...
                      {reviewer?.avatar && !avatarPreview && (
                        <Button
                          onClick={() => setAvatarPreview(reviewer.avatar)}
                          className="rounded-lg"
                        >
                          Restore Current
                        </Button>
                  ...
                  <div className="text-sm text-gray-600 space-y-2">
                    ...
                    <p>
                      ...
                      <span className="font-medium">
                        {
                          (
                            form.getFieldValue("assignedTeachers") ||
                            reviewer?.assignedTeachers ||
                            []
                          ).length
                        }{" "}
                        selected
