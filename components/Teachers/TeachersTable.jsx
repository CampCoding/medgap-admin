...
const TeachersTable = ({
  ...
  const filteredRows = useMemo(() => {
    ...
    if (selectedStatus && selectedStatus !== "all") {
      rows = rows.filter(
        (r) => toStatus(r.status) === toStatus(selectedStatus)
      );
    }
  ...
  const columns = [
    ...
    },
    {
      ...
      key: "phone",
      render: (phone) => (
        <Text className="text-gray-700">{phone || "N/A"}</Text>
      ),
    },
    {
      title: "Status",
    ...
    },
    {
      ...
      render: (_, record) => (
        <Space size="small">
          ...
          {/* 🔽 More dropdown (Edit / Change Status / Modules / Delete) */}
          <Dropdown
            trigger={["click"]}
              ...
              <Badge
                ...
                text={
                  <span className="capitalize font-medium text-lg">
                    {statusIcon(selectedTeacher.status)}{" "}
