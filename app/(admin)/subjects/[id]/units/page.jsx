...
const Units = () => {
  ...
  const subjectQS = subjectQuery ? `?subject=${encodeURIComponent(subjectQuery)}` : "";
  ...
  const tableActionMenu = (record) => ({
    ...
    onClick: ({ key }) => {
      ...
      if (key === "topics") return router.push(`${base}/topics${subjectQS}`);
      if (key === "questions") return router.push(`${base}/questions${subjectQS}`);
      if (key === "flashcards") return router.push(`${base}/flashcards${subjectQS}`);
      if (key === "edit") {
      ...
      render: (_, record) => (
        <div className="flex items-center gap-1">
          ...
          <Button
            ...
            onClick={() =>
              router.push(`/subjects/${id}/units/${record.id}/topics${subjectQS}`)
            }
            title="View topics"
          >
